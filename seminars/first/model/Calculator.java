...
public class Calculator {
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        if (discountAmount< 0 | discountAmount>100) {
            throw new ArithmeticException("ВВеден не допустимый размер скидки, скидка должны быть в диапазоне от 0 до 100%");
        }
        if (purchaseAmount < 0){
            throw new ArithmeticException("ВВедена отрицательная сумма покупки");
        }
